...
void GenomeAnnoDisplayView::paintGenesInPainter(QPainter *painter)
{
    ...
    // painter->drawLine(xpos,ceny0,width+xpos,ceny0);
    ...
    if(currentMaxGenei-currentMinGenei>2000){
        ...
        // painter->fillRect(QRectF(xpos,painty0,width,geneModelHeight),QBrush(colorNullStrandGene));
...
void GenomeAnnoDisplayView::paintGenesModelInPainter(QPainter *painter)
{
    ...
    long double tPaintStart,tPaintStop;
    long double positionGapLenF=tmpCurrentRightPosF-tmpCurrentLeftPosF;
    //int paintAreaWidth=width0-widgetLeftMargin; //width

    painter->setPen(QPen(Qt::black,2));
    // painter->drawLine(xpos,ceny0,xpos+width,ceny0);
    painter->setPen(QPen(Qt::black,1));
    ...
    if(currentMaxGenei-currentMinGenei>1000){
        painter->setFont(fontZoomInToLook);
        painter->drawText(xpos,painty0,width,geneModelHeight,Qt::AlignVCenter|Qt::AlignHCenter,textZoomInToLook);
        // painter->fillRect(QRectF(xpos,painty0,width,geneModelHeight),basicGeneBrush);
        painter->restore();
        return;
    }
    ...
    for(i=currentMinGenei;i<=currentMaxGenei;++i){
        ...
        while(iter!=pCDSList->end()){
            ...
            if((long double)tPaintStart>tmpCurrentRightPosF||
               (long double)tPaintStop<tmpCurrentLeftPosF) continue;
...
{
    if(tmpSelectedGenei<currentMinGenei||
       tmpSelectedGenei>currentMaxGenei||
...
//            qDebug() << QString::number(tBaseValue,2);
            ...
            if(tIBase==0){
                if(tStarti%2==0){
                    paintOneBase(base1,
                                 ((long double)tPos-tmpCurrentLeftPosF)*width/positionGapLenF+xpos,
                                 oneBaseLen,
                                 painter);
                    ++tPos;
                    paintOneBase(base2,
...
void GenomeAnnoDisplayView::paintGeneInfo(QPainter *painter)
{
    ...
    QString textPos=QString("Pos:")+QString::number(tGeneStart - cumChrLen[pGene->idChr])+".."+QString::number(tGeneStop - cumChrLen[pGene->idChr]);
    ...
    painter->drawText(px,py+6,
                      fw+6,fh,
                      Qt::AlignLeft|Qt::AlignTop,
                      textName);
    painter->setPen(QPen(Qt::black,1));
    painter->drawText(px,py+fh+6,
                      fw+6,fh,
                      Qt::AlignLeft|Qt::AlignTop,
                      textChr);
    painter->drawText(px,py+fh*2+6,
                      fw+6,fh,
                      Qt::AlignLeft|Qt::AlignTop,
                      textPos);
    painter->restore();
...
void GenomeAnnoDisplayView::paintDisplayAreaLabel(QPainter *painter)
{
    ...
    painter -> drawText(xpos - geneFW - displayAreaLabelGap,
                        geneCeny0 - FH/2,
                        geneFW,
                        FH,
                        Qt::AlignRight|Qt::AlignVCenter,
                        geneStr);
    painter -> drawText(xpos - geneModelFW - displayAreaLabelGap,
                        geneModelCeny0 - FH/2,
                        geneModelFW,
                        FH,
                        Qt::AlignRight|Qt::AlignVCenter,
                        geneModelStr);
    painter -> drawText(xpos - seqFW - displayAreaLabelGap,
                        baseCeny0 - FH/2,
                        seqFW,
                        FH,
                        Qt::AlignRight|Qt::AlignVCenter,
