...
GeneAnnotation::GeneAnnotation(const char* inName, int inIdChr, long long inStart, long long inStop, char inStrand,
                               const char* inAnnotation, const std::list<long long>& inCDS, long long inStartUTR5,
                               long long inStopUTR5, long long inStartUTR3, long long inStopUTR3)
    : idChr(inIdChr), start(inStart), stop(inStop), strand(inStrand),
    startUTR5(inStartUTR5), stopUTR5(inStopUTR5), startUTR3(inStartUTR3), stopUTR3(inStopUTR3)
{
    name = new char[charMaxLen];
    int i;
...
{
    if (inStart < start || inStart > stop || inStop < start || inStop > stop) {
        return false;
    }
...
{
    if (inStartUTR5 < start || inStartUTR5 > stop || inStopUTR5 < start || inStopUTR5 > stop) {
        return false;
    }
...
{
    if (inStartUTR3 < start || inStartUTR3 > stop || inStopUTR3 < start || inStopUTR3 > stop) {
        return false;
    }
...
long long GenomeAnnotation::search(const char *inKeywords) const
{
    ...
    for (i = 0; i < maxCharLim; i++) {
        badChar[i] = -1;
    }
    for (i = 0; i < kLen; i++) {
        badChar[int(inKeywords[i])] = i;
    }
    for (i = 0; i < kLen + 1; i++) {
        goodShift[i] = 0;
    }
    ...
    for (iGenes = 0; iGenes < (long long)(nGenes); ++iGenes) {
        ...
        while (i <= (sLen - kLen)) {
            j = kLen - 1;
            while (j >= 0 && inKeywords[j] == source[i + j]) {
                j--;
            }
            if (j < 0) {
                break;
            } else {
                i += std::max(goodShift[j + 1], std::max(1, j - badChar[int(source[i + j])]));
            }
        }
        if (i <= (sLen - kLen)) {
            break;
...
long long GenomeAnnotation::search(int inChrI, long double position, unsigned short mode) const
{
    ...
    while (i <= j) {
        m = (i + j) / 2;
        if (mode == GA_START) {
            if ((long double)(genes[m].start) >= position) {
                j = m - 1;
            } else {
                i = m + 1;
            }
        } else {
            if ((long double)(genes[m].stop) >= position) {
                j = m - 1;
            } else {
                i = m + 1;
            }
...
long long GenomeAnnotation::search(int inChrI, long long position, unsigned short mode) const
{
    ...
    while (i <= j) {
        m = (i + j) / 2;
        if (mode == GA_START) {
            if (genes[m].start >= position) {
                j = m - 1;
            } else {
                i = m + 1;
            }
        } else {
            if (genes[m].stop >= position) {
                j = m - 1;
            } else {
                i = m + 1;
            }
...
GenomeSequence::GenomeSequence(char** inChrName, int inNChr, const long long* inChrLen)
    : GenomeSequence()
{
...
void GenomeSequence::setInitialization(char** inChrName, int inNChr, const long long* inChrLen)
{
    long long i, j;
    clear();
    nChr = inNChr;
...
{
    if (iChr >= nChr || iPosition < 0) {
        return false;
    }
    if (chrLen[iChr] < iPosition) {
        return false;
    }
    ...
    while (inSeq[i] && (i + iPosition) <= chrLen[iChr]) {
        i3 = (iPosition - 1) / 2 + i2;
        if (!flag) {
            chrSeq[iChr][i3] = (Base2Value(inSeq[i]) << 4) | (chrSeq[iChr][(iPosition - 1) / 2 + i2] & char(0b1111));

        } else {
            chrSeq[iChr][i3] = Base2Value(inSeq[i]) | (chrSeq[iChr][(iPosition - 1) / 2 + i2] & char(0b11110000));
            ++i2;
        }
        flag = (flag + 1) % 2;
        ++i;
...
void GenomeSequence::write(const char *file,long long ip)
{
    ...
    for (long long i = 0; i < nChr; ++i) {
        ...
        for (long long j = ip; j <= chrLen[i]; ++j) {
            ...
            b1 = tmp & 0b1111;
            if (j % 2 == 0) {
                outLine += Value2Base(b1);
            } else {
                outLine += Value2Base(b2);
            }
        }
        oF << outLine << '\n';
    }
