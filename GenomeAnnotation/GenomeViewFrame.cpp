...
bool GenomeViewFrame::mouseXToPointX(int inMouseX1, int inMouseX2, long double &outLeftX, long double &outRightX)
{
    ...
    if (inMouseX1 > inMouseX2) {
        ...
        if (pos1 - pos2 + 1 < minIntervalBaseNum) {
            return false;
        }
        outLeftX = pos2;
        outRightX = pos1;
        ...
        pos1 = double(inMouseX1 - widgetLeftMargin) / (width0 - widgetLeftMargin) * positionGapLen + tmpCurrentLeftPosF;
        pos2 = double(inMouseX2 - widgetLeftMargin) / (width0 - widgetLeftMargin) * positionGapLen + tmpCurrentLeftPosF;
        if (pos2 - pos1 + 1 < minIntervalBaseNum) {
            return false;
        }
        outLeftX = pos1;
        outRightX = pos2;
...
void GenomeViewFrame::paintVerticalLineInPainter(QPainter *painter)
{
    ...
    painter->fillRect(QRectF(widgetLeftMargin, posLabely, width0 - widgetLeftMargin, positionLabelHeight),
        QBrush(QColor("#bbbbbb")));
        ...
        while ((long double)k <= tmpCurrentRightPosF) {
            if ((long long)j <= currentMaxChri && k > (long long)cumChrLen[j]) {
                ...
                if ((long double)k >= tmpCurrentLeftPosF && k - cumChrLen[j - 1]) {
                    ...
                    painter->drawText(tVLineX - positionLabelWidth / 2, posLabely,
                        positionLabelWidth, positionLabelHeight, Qt::AlignCenter, posStr);
                }
                k += scatterVLineGap1;
            }
        }
...
void GenomeViewFrame::paintChrGridLineInPainter(QPainter *painter)
{
    ...
    for (int i = 0; i < nChr; ++i) {
        ...
        painter->drawText(tChrNameX - positionLabelWidth / 2, posLabely,
            positionLabelWidth, positionLabelHeight, Qt::AlignCenter, chrStr);
    }
    painter->restore();
}

void GenomeViewFrame::paintZoomRegionLineInPainter(int leftMousePressedPosX, int leftMouseMovingPosX, QPainter *painter)
{
    QPen tPen;
...
void GenomeViewFrame::paintBackground(QPainter *painter)
{
    ...
    if (width0-widgetLeftMargin > 300) {
        painter->drawText(widgetLeftMargin, 0, width0 - widgetLeftMargin, widgetTopMargin,
            Qt::AlignHCenter | Qt::AlignVCenter, "Slide Zoom Area");
    }
