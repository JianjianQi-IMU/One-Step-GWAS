...
void LogPScatterViewPainter::initInitializeGL()
{
    ...
    tmpPaintWidget->setGLShaderProgram(":/shader/pointsVertex.shader",
                                       ":/shader/pointsFragment.shader");
...
{
    if(!isDisplayMovingLogPLine || tmpMovingMouseY > displayArea.y() + displayArea.height()
            || tmpMovingMouseY < displayArea.y())
    {
        return;
    }
...
void LogPScatterViewPainter::sendPointsToBuffer()
{
    ...
    while(ta<=tb){
        mid=(ta+tb)/2;
        if((long double)(cumPointsPos[mid])>=tmpCurrentLeftPosF) tb=mid-1;
        else ta=mid+1;
    }
    ...
    while(ta<=tb){
        mid=(ta+tb)/2;
        if((long double)(cumPointsPos[mid])>tmpCurrentRightPosF) tb=mid-1;
        else ta=mid+1;
    }
...
void LogPScatterViewPainter::paintAxisInPainter(QPainter *painter)
{
    ...
    for(long long i=intDownLogP;i<=intUpLogP;i+=logPStride){
        ...
        painter->drawLine(
                        xpos,
                        painty,
                        xpos-LogPAxisScaleLen,
                        painty);
        painter->drawText(
                        QRectF(1,painty-20,xpos-LogPAxisScaleLen-5,40),
...
{
    if(selectedPointi<currentMinPointsi||selectedPointi>currentMaxPointsi) return;
    int px, py;
    painter->save();
...
{
    if(idPoint<currentMinPointsi||idPoint>currentMaxPointsi) return;
    int px, py;
...
void LogPScatterViewPainter::paintLabelPoints(QPainter *painter)
{
    long long tPos;
    int px,py;
    if(idLabelPoints.size() == 0) return ;
    painter->save();
