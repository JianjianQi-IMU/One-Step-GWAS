...
bool LogPScatterVisualizeWidget::loadPoints(ValPoints *inPoints, long long inNPoints)
{
    ...
    if(flag){
        pThresholdPainter -> preSetValueRange(pPointsPainter->getIntDownLogP(),
                                              pPointsPainter->getIntUpLogP());
        pThresholdPainter -> preSetPoints(pPointsPainter->getPoints(),
                                          pPointsPainter->getNPoints());
    }
    return flag;
}
...
void LogPScatterVisualizeWidget::leftMousePressEvent(QMouseEvent *e,bool& isNeedUpdate)
{
    ...
    if(mouseY<=widgetTopMargin){if(mouseX>=widgetLeftMargin)
        {
...
void LogPScatterVisualizeWidget::makeMenu()
{
    ...
    connect(actAddLabelPoint,&QAction::triggered,
            this,&LogPScatterVisualizeWidget::dealAddPointLabel);
    connect(actremoveLabelPoint,&QAction::triggered,
            this,&LogPScatterVisualizeWidget::dealRemovePointLabel);
...
void LogPScatterVisualizeWidget::paintGL()
{
    ...
    pAnnoPainter  -> preSetDisplayArea(QRect(widgetLeftMargin,
                                             widgetTopMargin + scatterAreaHeight,
                                             width0 - widgetLeftMargin,
                                             annoAreaHeight));
    pPointsPainter -> preSetDisplayArea(QRect(widgetLeftMargin,
                                              widgetTopMargin,
                                              width0 - widgetLeftMargin,
                                              scatterAreaHeight));
    pPointsPainter -> preSetWidgetSize(size());
    pPointsPainter -> preSetCurrentPosF(currentLeftPosF,currentRightPosF);
    pThresholdPainter -> preSetDisplayArea(QRect(widgetLeftMargin,
                                                 widgetTopMargin,
                                                 width0 - widgetLeftMargin,
                                                 scatterAreaHeight));
    ...
    pFramePainter->paintGenomeViewFrame(isLeftMousePressed,
                                      leftMousePressedPosX,
                                      leftMouseMovingPosX,
                                      &Painter);
...
void LogPScatterVisualizeWidget::savePaintCurrentSVG(QString savePath,
                                                     QSizeF inSize,
