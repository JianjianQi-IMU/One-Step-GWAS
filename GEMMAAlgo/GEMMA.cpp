...
GEMMA::MLM::MLM(const MML::Phenotype&  inPhe, const MML::Kinship& inKin, const MML::Covariates& inCov)
    : MLM()
{
...
bool GEMMA::MLM::analyze(const MML::Mat& Utx, double& outBetax, double& outFstat)
{
    if (!isValid || (Utx.info != MML::_colvec && Utx.info != MML::_rowvec) ||
        Utx.getNCol() * Utx.getNRow() != filterN) {
        return false;
    }
...
bool GEMMA::MLM::analyze(const double* Utx, double& outBetax, double& outFstat)
{
...
bool GEMMA::MLM::calcuPPatb(const MML::Mat& D, const MML::Mat& atPb, MML::Mat& atPPb)
{
...
bool GEMMA::MLM::calcuPPPatb(const MML::Mat& D, const MML::Mat& atPb, const MML::Mat& atPPb, MML::Mat& atPPPb)
{
...
bool GEMMA::MLM::calcuTrP(double lambda, const MML::Mat& atPb, const MML::Mat& atPPb, double& outTrP)
{
...
bool GEMMA::MLM::calcuTrPP(double lambda, const MML::Mat& atPb, const MML::Mat& atPPb, const MML::Mat& atPPPb, double& outTrPP)
{
    long long i = 0, ii = 0, yii = 0;
    outTrPP = 0;
...
bool GEMMA::MLM::calcuREMLLogL(double lambda, const MML::Mat& atPb, const MML::Mat& atIb, double& outLogL)
{
    double tmp = 0.0, Ldelta = 0.0, ytPy = 0.0, LytPy = 0.0;
    double LKxtKx = 0.0, LKxtDiKx = 0.0;
    long long i = 0, j = 0, ii = 0;
    for (i = 1; i <= factorN + 1; ++i) {
        ii = (i - 1) * i / 2 + i - 1;
        if (std::fabs(atPb(i - 1, ii)) < eps || std::fabs(atIb(i - 1, ii)) < eps) {
            return false;
        }
    ...
    if (std::fabs(ytPy) < eps) {
        return false;
    }
    LytPy = std::log(ytPy);
    for (i = 0; i < filterN; ++i) {
        if (std::fabs(delta(i)) < eps) {
            return false;
        }
...
bool GEMMA::MLM::calcuREMLDLogL1(double lambda,double trP, const MML::Mat& atPb, const MML::Mat& atPPb,
    double& outDLogL1)
{
    long long yii = 0;
...
bool GEMMA::MLM::calcuREMLDLogL2(double lambda, double trP, double trPP, const MML::Mat& atPb,
    const MML::Mat& atPPb, const MML::Mat& atPPPb, double& outDLogL2)
{
    long long yii = 0;
...
bool GEMMA::MLM::calcuLambda(double& outLambda, double& outLL)
{
    ...
    if (choosedNum == 0) {
        if ((Lpara.dll1List)[0] > 0) {
            tmpLambda = std::exp((Lpara.limList)[Lpara.n]);
        } else {
            tmpLambda = std::exp((Lpara.limList)[0]);
        }
        D = delta;
        D.mul(tmpLambda);
        D.add(1);
...
bool GEMMA::MLM::calcuFWald(double lambda, double& outBetax, double& outFWald)
{
    MML::Mat atPb, D;
...
GEMMA::GLM::GLM(const MML::Phenotype& inPhe, const MML::Covariates& inCov)
    : GLM()
{
    read(inPhe, inCov);
}
...
bool GEMMA::GLM::read(const MML::Phenotype& inPhe, const MML::Covariates& inCov)
{
...
bool GEMMA::GLM::analyze(const MML::Mat& x, double& outBetax, double& outFstat)
{
    if (!isValid || (x.info != MML::_colvec && x.info != MML::_rowvec) ||
       x.getNCol() * x.getNRow() != filterN) {
        return false;
    }
    MML::Mat Ktx, KtKiKtx;
    double xtx, xtKKtKiKtx, xty, xtKKtKiKty, xPKx, xPKy, B, VarB;
    if (!MML::Mat::XVmul(Kt, x, Ktx)) {
        return false;
    }
    if (!MML::Mat::XVmul(KtKi, Ktx, KtKiKtx)) {
        return false;
    }
    if (!MML::Mat::VtUmul(KtKiKtx, Ktx, xtKKtKiKtx)) {
        return false;
    }
    if (!MML::Mat::VtUmul(KtKiKty, Ktx, xtKKtKiKty)) {
        return false;
    }
    if (!MML::Mat::VtUmul(x, x, xtx)) {
        return false;
    }
    if (!MML::Mat::VtUmul(x, y, xty)) {
        return false;
    }
    xPKx = xtx - xtKKtKiKtx;
    xPKy = xty - xtKKtKiKty;
