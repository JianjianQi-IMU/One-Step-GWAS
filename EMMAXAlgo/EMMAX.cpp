...
EMMAX::REML::REML(const MML::Kinship& inKin,
...
void EMMAX::REML::qSortEigen(size_t a,size_t b,size_t* outIndex){
    ...
    if(a<b){
        ...
        tmp=outIndex[i];
        while(i<j){
            while(i<j&&rawEigenVal(outIndex[j])>=rawEigenVal(tmp)) --j;
            outIndex[i]=outIndex[j];
            while(i<j&&rawEigenVal(outIndex[i])<rawEigenVal(tmp)) ++i;
...
bool EMMAX::REML::read(const MML::Kinship&    inKin,
                       const MML::Covariates& inCov){
    if(!(inKin.isValid)||
       !(inCov.isValid)||
    ...
    // std::cout << "N:" << filterN << std::endl;
    // std::cout << "q:" << factorN << std::endl;
    ...
    // filterEigenVal.print();
    // std::cout << "step 4" << std::endl;
    // std::cout << "step 3" << std::endl;
    // std::cout << "OUTT:" << std::endl;
    // MML::Mat OUTT,tfilterEigenVec;
    // filterEigenVec.t(tfilterEigenVec);
    // MML::Mat::XYmul(tfilterEigenVec,filterEigenVec,OUTT);
    // OUTT.print();
    isValid=true;
    return true;
}

bool EMMAX::REML::getDelta(const MML::Phenotype& inPhe,
                           double&               outDelta,
                           double&               outLLD  ){
    if(!(inPhe.isValid)      ||
       inPhe.filterN!=filterN||
    ...
    // EVect.print();
    // MML::Mat OUTT;
    // MML::Mat::XYmul(EVect,filterEigenVec,OUTT);
    // OUTT.print();
    ...
    double upVal=0.0,downVal=0.0,midVal=0.0,halfDiff=0.0,dLLMid=0.0;
    ...
    if(dLikelihood(etas,expRange[rangeNum],dLL[rangeNum])){
        if(dLL[rangeNum]>0){
            delete [] expRange;
            delete [] dLL;
            // std::cout << "dll[rangeNum]>0:" << dLL[rangeNum] << std::endl;
...
bool EMMAX::REML::dLikelihood(const MML::Mat& etas,double delta,double& outVal){
    ...
    // std::cout << delta << '\t' << outVal << std::endl;
    return true;
}
...
EMMAX::Betas::Betas(){isValid=false;eps=MML::EPS;}

EMMAX::Betas::Betas(const MML::Phenotype&  inPhe,
...
bool EMMAX::Betas::read(const MML::Phenotype&  inPhe,
                        const MML::Kinship&    inKin,
                        const MML::Covariates& inCov,
                        double                 inDelta){
    if(inPhe.filterN!=inKin.filterN||
...
bool EMMAX::Betas::calculateTstat(const MML::Mat& inMarkers,
                                  double&         outBetas0,
                                  double&         outTstat){
    if(inMarkers.info==MML::_null            ||
       !isValid                                   ||
       inMarkers.info!=MML::_colvec  ) return false;
    ...
    for(size_t i=0;i<filterN;++i) UtKx(i,factorN)=Utx(i);
    // std::cout << "step 2" << std::endl;
    // std::cout << UtKx.info << '\t' << D.info << std::endl;
