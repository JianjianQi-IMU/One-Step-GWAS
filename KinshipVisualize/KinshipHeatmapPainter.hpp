...
public:
    ...
    double operator ()(long long a, long long b) {return dis(a,b);}
    double operator ()(long long id) {return val(id);}
};
...
public:
    ...
    void setMatView(const QMatrix4x4& mat){matView = mat;}
    void setRangeValue(double highVal, double lowVal, double middleVal)
    {
        maxVal = highVal,minVal = lowVal;
        meanVal = middleVal;
    }
    void setLowColor(const QColor& col){lowColor = col;}
    void setHighColor(const QColor& col){highColor = col;}
    ...
    void setMeanColor(const QColor& col){meanColor = col;}
    void setOnePointSize(double inSize){oneSize = inSize;}
    ...
    double getMeanVal() const{return meanVal;}
    QColor getLowColor() const{return lowColor;}
    QColor getHighColor() const{return highColor;}
    QColor getMeanColor() const{return meanColor;}
    long long getNIndividuals() const{return kin.getNRow();}

    void setClusterIndex();
    void isClickedPoint(const QPoint& point,
                        long long &outRowIndex,
                        long long &outColIndex)
    {
        outRowIndex = -1;
        outColIndex = -1;
    ...
    QRectF calcuPointRectPix(long long idRow,long long idCol)
    {
        ...
        return QRectF(rect.x()*kin.getNRow()*oneSize,
                     rect.y()*kin.getNRow()*oneSize,
                     rect.width()*kin.getNRow()*oneSize,
                     rect.height()*kin.getNRow()*oneSize);
    }
...
private:
    ...
    {
        long long nInd = kin.getNRow();
        if(rowSelected < 0 || rowSelected >= nInd ||
           colSelected < 0 || colSelected >= nInd){
            return;
        }
