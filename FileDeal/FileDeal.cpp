    ...
    while (std::getline(file, line)) {
        ...
        for (iSample = 0; iSample < nSample; ++iSample) {
            if (lineList[iSample].size() == 2) {
                if (!(lineList[iSample] == "NA" || lineList[iSample] == "nA" ||
                    lineList[iSample] == "na" || lineList[iSample] == "Na")) {
...
FD::BIMLogPread::BIMLogPread(const char *inFile)
    : BIMLogPread()
{
    load(inFile);
}
...
bool FD::BIMLogPread::readAll(BIMLogPData& out)
{
    ...
    if (out.chrName) {
        for (int i = 0; i < out.nChr; ++i) {
            delete [] out.chrName[i];
        }
        delete [] out.chrName;
    }
    out.chrName = nullptr;
...
bool FD::BIMread::readAll(BIMData& out)
{
    ...
    out.isValid = false;
    for (i = 0; i < 6; ++i) {
        lineList[i] = new char[MAXELEMLEN];
    }
    std::set<std::string> chrSet;
    std::map<std::string, int> chrMap;
    size_t nMarker = 0;
...
bool FD::FAMread::clear()
{
    ...
    if (data) {
        for (long long i = 0; i < 6 * sampleN; ++i) {
            delete [] data[i];
        }
        delete [] data;
    }
...
bool FD::FAMread::readAll()
{
    ...
    for (i = 0; i < sampleN * 6; ++i) {
        data[i] = new char[MAXELEMLEN];
    }
...
bool FD::genomeRead::readTsv()
{
    ...
    if (!errorFlag) {
        ...
        while (!(file.eof())) {
            ...
            if (idChrMap.find(lineList[1]) == idChrMap.end()) continue;
            if (nList == 6) {
                *(genome->getGene(iGenes)) = GeneAnnotation(lineList[0], idChrMap[lineList[1]],
                    std::stoull(lineList[2]), std::stoull(lineList[3]), lineList[4][0], lineList[5]);
            } else if (nList == 5) {
                *(genome->getGene(iGenes)) = GeneAnnotation(lineList[0], idChrMap[lineList[1]],
                    std::stoull(lineList[2]), std::stoull(lineList[3]), lineList[4][0], "NA");
            }
            ++iGenes;
        }
    }
...
bool FD::genomeRead::readGff()
{
    ...
    while (!(file.eof())) {
        ...
        if (geneTypeLabel == lineList[2]) {
            if (readBegin) {
                *(genome->getGene(iGenes)) = GeneAnnotation(tGeneId.c_str(), idChrMap[tChr], tStart,
                    tStop, tStrand, tGeneAnno.c_str(), tCDSList, tStart5UTR, tStop5UTR, tStart3UTR, tStop3UTR);
                ++iGenes;
            }
    ...
    if (iGenes > 0 && (!errorFlag)) {
        *(genome->getGene(iGenes)) = GeneAnnotation(tGeneId.c_str(), idChrMap[tChr], tStart, tStop,
            tStrand, tGeneAnno.c_str(), tCDSList, tStart5UTR, tStop5UTR, tStart3UTR, tStop3UTR);
...
bool FD::pheRead(const char *inFile, MML::Mat &out)
{
    ...
    while (!(iF.eof())) {
        ...
        if (compare(line, "NA\0") || compare(line, "na\0")) {
                       ...
                       GenomeSequence& genomeSeq)
{
    ...
    while (!(file.eof())) {
        ...
        if (line[0] == '>') {
            i = 1;
            while (i < MAXLINELEN - 1 && line[i] != '\0' && line[i]!=' '&&
                line[i] != '\t' && line[i] != '\n' && line[i] != '\r') {
                tChr[i - 1] = line[i];
                ++i;
            }
            tChr[i - 1] = '\0';
            if (chrMap.find(tChr) == chrMap.end()) {
                flag = false;
            } else {
                flag = true;
                chrLen[tChr] = 0;
    ...
    while (!(file.eof())) {
        ...
        if (line[0] == '>') {
            i = 1;
            while (i < MAXLINELEN - 1 && line[i] != '\0' && line[i] != ' ' && line[i] != '\t' &&
                line[i] != '\n' && line[i] != '\r') {
                tChr[i - 1] = line[i];
                ++i;
            }
    ...
    while (!(file.eof())) {
        ...
        if(compare(lineList[logPCol], "nan") ||
           ...
           compare(lineList[logPCol], "na")) {
            ...
            data[iMarkers] = atof(lineList[logPCol]);
        }
