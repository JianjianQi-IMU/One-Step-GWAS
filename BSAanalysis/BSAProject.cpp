...
BSAVCFRead::BSAVCFRead(const char *inFile)
    : BSAVCFRead()
{
    load(inFile);
}
...
bool BSAVCFRead::readAll(MML::Mat &data, std::vector<std::string> &poolName,
    ValPoints2 *&points, std::vector<std::string> &chrName)
{
    if (!isValid) {
        return false;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (line[0] != '#') {
            ...
            for (iFormat = 0; iFormat < formatList.size(); ++iFormat) {
                if (formatList[iFormat] == "AD") {
                    iAD = iFormat;
                } else if (formatList[iFormat] == "GT") {
                    iGT = iFormat;
                }
            }
            if (iAD < 0 || iGT < 0) {
                ...
                if (infoList[iGT].find('.') != std::string::npos ||
                   infoList[iAD].find('.') != std::string::npos) {
                    ...
                    for (i = 0; i < gtList.size(); ++i) {
                        if (gtList[i] == "0") {
                            tAD1 += std::stoull(adList[i]);
                        } else {
                            tAD2 += std::stoull(adList[i]);
                        }
                    }
                    data(iMarker, 0) = tAD1;
                    data(iMarker, 1) = tAD2;
...
bool BSASpecificRead1::readAll(MML::Mat &data, std::vector<std::string> &poolName, ValPoints2 *&points, std::vector<std::string>& chrName)
{
...
bool BSAProject::readFileVCF(const char *inFile)
{
    ...
    sortData = false;
    readPoints = false;
    if (!(readerVCF.load(inFile))) {
        return false;
    }
    if (!(readerVCF.readAll(data, poolName, points, chrName))) {
        return false;
    }
    maxDepth = 0;
    for (size_t iMarker = 0; iMarker < data.getNRow(); ++iMarker) {
...
bool BSAProject::readFile1(const char *inFile)
{
    ...
    sortData = false;
    readPoints = false;
    if (!(reader1.load(inFile))) {
        return false;
    }
    if (!(reader1.readAll(data, poolName, points, chrName))) {
        return false;
    }
    maxDepth = 0;
    for (size_t iMarker = 0; iMarker < data.getNRow(); ++iMarker) {
